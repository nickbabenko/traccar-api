    ...
    this.msgBufferRaw.forEach((msg, idx) => {
        switch (selectEvent(msg).number) {
            ...
            case 0x13: // status message
                ...
                break;
            case 34:
              Object.assign(parsed, parseX1(msg), { imei: this.imei });
              break;
            // case 0x15:
            //     //parseLocation(msg);
            //     break;
...
function parseX1(data) {
  ...
  const read = (length) => {
    try {
      ...
      switch (length) {
        case 1:
          return result.readUInt8()
        case 2:
          return result.readUInt16BE()
        case 4:
          return result.readUInt32BE()
      }
      return result
    } catch (e) {
      return null
  ...
  let result = {
    alarm: { type: read(1) },
    date: parseDatetime(read(6)).toISOString(),
    gpsSatellites: (read(1) & 0xF0) >> 4,
    latitude: read(4),
    longitude: read(4),
    speed: read(1),
    course: read(2),
    terminal: read(2),
    odometer: read(4),
    mobileCountryCode: read(2),
    mobileNetworkCode: read(1),
    locationAreaCode: read(2),
    cellTowerId: read(4),
    rfid: read(4),
  }

  result.latitude = decodeGt06Lat(result.latitude, result.course)
  result.longitude = decodeGt06Lon(result.longitude, result.course)
  result.course = result.course & 0x3FF
